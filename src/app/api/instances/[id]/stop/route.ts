...
export async function POST(
  ...
  try {
    if (inst.providerInstanceId) {
      ...
      await provider.stopInstance(inst.providerInstanceId);
    }
    await db
      .update(instances)
      .set({ status: "stopped" })
    ...
    console.error("Failed to stop instance", id, e);
    const message =
      e instanceof Error ? e.message : "Provider failed to stop instance";
